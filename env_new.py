...
class MultiUAVEnv(object):
    def __init__(self):
        ...
        self.initial_uav_energy = 300  # 初始UAV能量
        self.uav_energy = np.array([self.initial_uav_energy] * self.uav_num)  # UAV能量
        self.uav_cache_energy = 5
        self.uav_trans_energy = 1
        self.ue_energy_list = np.random.uniform(0.2, 1, self.ue_num)
        ...
        self.action_dim = self.uav_num * 3
    ...
    def get_initial_state(self):
        ...
        state = np.append(state, self.ue_energy_list)
        return state
    ...
    def step(self, action, step):
        action = (action + 1) / 2  # 将取值区间从 [-1,1] 映射到 [0,1]
        total_reward = 0

        # 更新用户位置
        self.ue_location_list = update_user_positions(self.ue_location_list, user_move_distance, self.area_size)
        ...
        for i, uav in enumerate(self.uav_location):
            for dx in range(-20, 21):
                for dy in range(-20, 21):
                    ...
                    new_position = np.clip(new_position, 0, self.area_size)  # 限制无人机位置在区域内
                    temp_positions = self.uav_location.copy()
                    temp_positions[i] = new_position  # 临时存储移动后的无人机位置

                    # 计算覆盖的用户数量，确保不重复覆盖
                    coverage = calculate_coverage(temp_positions, self.ue_location_list, self.uav_range)  # 计算覆盖情况
        ...
        # 计算当前每个无人机覆盖的用户数量
        coverage = calculate_coverage(self.uav_location, self.ue_location_list, self.uav_range)  # 计算覆盖情况
        individual_coverage = np.sum(coverage, axis=1)  # 计算每个无人机覆盖的用户数量

        # 输出调试信息
        # print(f"时隙 {step + 1}:，UAV覆盖用户的数量：{individual_coverage}")
        for i in range(self.uav_num):
            covered_users = np.where(coverage[i])[0]
            # print(f"  UAV {i + 1} 覆盖的用户: {covered_users}")
        ...
        for uav_index in range(self.uav_num):
            ...
            uav_energy_consumption = self.coefficient * self.uav_f ** 2 * self.W_list[ue_index] * computation_ratio *0.01
            ue_energy_consumption = self.coefficient * self.ue_f ** 2 * self.W_list[ue_index]
            ...
            if step % SLOT_CYCLE == 0:
                ...
                self.uav_energy[uav_index] -= self.uav_cache_energy
            ...
            if is_offloading:
                ...
                if uav_delay < bs_delay and has_cache and self.uav_energy[uav_index] > uav_energy_consumption:
                    ...
                    delay = bs_delay
                    delay_source = '基站'
                    self.uav_energy[uav_index] -= self.uav_trans_energy
            else:
                # 本地计算
                local_delay = self.local_delay(ue_index)
                delay = local_delay
                delay_source = '本地'
                self.ue_energy_list[ue_index] -= ue_energy_consumption

            reward = -delay
            # 输出调试信息
            #print(f"无人机 {uav_index + 1} 选择用户 {ue_index}，是否在服务范围内：{in_coverage}，是否卸载: {is_offloading}，是否具有缓存: {has_cache}，分配计算资源：{computation_ratio}，计算时延来源: {delay_source}，时延：{delay}")
